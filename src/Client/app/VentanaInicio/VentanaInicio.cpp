...
void VentanaInicio::obtenerEntrada(){
	...
	while( !terminar && !terminoEntrada){
		...
		this->texturaCantidadJugadores = cargoTextura("Conectados "+ to_string(jugadoresConectados)+"/"+to_string(jugadoresTotales)+" jugadores", colorBlanco,this->renderer,this->fuente);
		...
		if(ingresoIncorrectoCredenciales){
			...
			if (informacionJugadoresConectados.tope >= jugadoresTotales){
				salaLlena = true;
				this->texturaMensajeSalaLlena = cargoTextura("La sala esta llena, no puede ingresar", colorRojo,this->renderer,this->fuente);
				...
				renderizar(375,270,18,250,texturaMensajeSalaLlena,renderer);

			}else{

				this->texturaMensajeCredencialesIncorrectas = cargoTextura("Las credenciales ingresadas son erroneas", colorRojo,this->renderer,this->fuente);
				if( this->texturaMensajeCredencialesIncorrectas == nullptr ){
					...
					renderizar(375,270,18,250,texturaMensajeCredencialesIncorrectas,renderer);
...
void VentanaInicio::imprimirMensajeEspera(){
	...
	*/
	...
	this->texturaMensajeCredencialesCorrectas = cargoTextura("Credenciales correctas", colorVerde,this->renderer,this->fuente);
	...
	this->texturaMensajeEspera = cargoTextura("Esperando al resto de jugadores", colorVerde,this->renderer,this->fuente);
